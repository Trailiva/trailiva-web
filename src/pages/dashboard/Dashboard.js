...
const Dashboard = () => {
  ...
  const [quote, setQuote] = useState({
    content: "Action takers are life makers",
    author: "unknown",
  });
  ...
  const handleViewTask = async (id) => {
    try {
      ...
      const { name, priority, description, createdAt, dueDate, tab } =
        task.data;
  ...
  return isLoading ? (
    ...
    <>
      ...
      <Grid container spacing={0}>
        <Grid item xs={2}>
          <Sidebar
            referenceName={workspace.referenceName}
        ...
        <TaskCreator
          validateTaskCreated={handleValidateTaskCreated}
          open={openCreateTaskModal}
          handleClosePopup={handleCloseCreateTaskModal}
        />
        <Grid
          container={true}
          item
                ...
                <>
                  ...
                  {sidebar.tasks && (
                    <TaskContainer
                      ...
                      isSuccessful={taskCreated}
                      onHandleClick={handleOpenCreateTaskModal}
                    />
                  )}

                  {sidebar.overview && (
                    <DailyQuote quote={quote?.content} author={quote?.author} />
                  )}
                  {sidebar.overview && (
                    <SetupContainer
                      setups={setups}
